...
with st.sidebar:
    ...
    market_probability_pct = st.slider(
        ...
        min_value=0,
        max_value=100,
        value=preset["market_probability"],
...
metric_2.metric("Model Probability", format_percent(result["model_probability"]))
metric_3.metric("Market Probability", format_percent(market_probability))
...
with left_col:
    ...
    st.write(f"**Headline score:** {result['event_score']:+.2f}")
...
with right_col:
    ...
    st.write("The prototype maps average agent sentiment into a model probability.")
    st.code(
